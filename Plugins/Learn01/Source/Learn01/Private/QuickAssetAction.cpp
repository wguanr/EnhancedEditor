...
void UQuickAssetAction::AddPrefix()
{
	...
	for (UObject* SelectedAsset:SelectedAssets)
	{
		...
		{
			Print(TEXT("Prefix doesnt exist for asset type :" + SelectedAsset->GetClass()->GetName()),FColor::Red);
			continue;
		}
		...
		{
			Print(TEXT("Prefix already exists for asset :"+ SelectedAsset->GetName()),FColor::Red);
			continue;
		}
	...
	ShowNotifyInfo(TEXT("I have renamed " + FString::FromInt(Counter) + " assets"));
...
void UQuickAssetAction::RemoveUnusedAssets()
{
	...
	{
		ShowMessageDialog(EAppMsgType::Ok,TEXT("Find nothing Unused"),false);
		return;
	}
