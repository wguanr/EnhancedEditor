...
void FEnhancedEditorModule::StartupModule()
{
    ...
    if (FModuleManager::Get().IsModuleLoaded(TEXT("ContentBrowser"))) {
        ...
        ContentBrowserModulesChangedHandle = FModuleManager::Get().OnModulesChanged().AddRaw(
            this, &FEnhancedEditorModule::OnModulesChanged);
    }
}
...
void FEnhancedEditorModule::ShutdownModule()
{
    ...
    if (ContentBrowserModulesChangedHandle.IsValid()) {
        FModuleManager::Get().OnModulesChanged().Remove(ContentBrowserModulesChangedHandle);
        ContentBrowserModulesChangedHandle.Reset();
    }

    if (FGlobalTabmanager::Get().GetSharedReferenceCount() > 0) {
...
{
    if (FContentBrowserModule* CBModulePtr = FModuleManager::Get().GetModulePtr<FContentBrowserModule>(TEXT("ContentBrowser"))) {
        FContentBrowserModule& CBModule = *CBModulePtr;
...
void FEnhancedEditorModule::OnModulesChanged(FName InModuleName, EModuleChangeReason InChangeReason)
{
    if (InModuleName == TEXT("ContentBrowser") && InChangeReason == EModuleChangeReason::ModuleLoaded) {
        IniCBMenuExtension();
        if (ContentBrowserModulesChangedHandle.IsValid()) {
            FModuleManager::Get().OnModulesChanged().Remove(ContentBrowserModulesChangedHandle);
            ContentBrowserModulesChangedHandle.Reset();
...
TSharedRef<FExtender> FEnhancedEditorModule::CustomCBMenuExtender(const TArray<FString>& SelectedPaths)
{
    ...
    if (SelectedPaths.Num() > 0) {
        ...
        MenuExtender->AddMenuExtension(FName("Delete"), EExtensionHook::After, TSharedPtr<FUICommandList>(), FMenuExtensionDelegate::CreateRaw(this, &FEnhancedEditorModule::AddCustomMenuEntry));
...
void FEnhancedEditorModule::OnCleanUselessShit()
{
    ...
    for (const FString& ShitPath : ShitPathInFolder) {
        ...
        if (UEditorAssetLibrary::DoesDirectoryExist(ShitPath) && !UEditorAssetLibrary::DoesDirectoryHaveAssets(ShitPath)) {
...
void FEnhancedEditorModule::OnDeleteUnusedFoldersButtonCliked()
{
    ...
    for (const FString& FolderPath : AssetsPathInFolder) {
        ...
        if (UEditorAssetLibrary::DoesDirectoryExist(FolderPath) && !UEditorAssetLibrary::DoesDirectoryHaveAssets(FolderPath)) {
