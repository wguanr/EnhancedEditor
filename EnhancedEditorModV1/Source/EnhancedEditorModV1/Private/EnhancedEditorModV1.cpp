...
void FEnhancedEditorModV1Module::IniCBMenuExtension()
{
	...
	CBMenuExtenders.Add(FContentBrowserMenuExtender_SelectedPaths::CreateRaw(this,&FEnhancedEditorModV1Module::CustomCBMenuExtender));
}
...
TSharedRef<FExtender> FEnhancedEditorModV1Module::CustomCBMenuExtender(const TArray<FString>& SelectedPaths)
{
	...
	if (SelectedPaths.Num()>0)
	{
		...
		MenuExtender->AddMenuExtension(FName("Delete"),EExtensionHook::After,TSharedPtr<FUICommandList>(),
			FMenuExtensionDelegate::CreateRaw(this,&FEnhancedEditorModV1Module::AddCustomMenuEntry));
...
void FEnhancedEditorModV1Module::OnCleanUselessShit()
{
	...
	for(const FString& ShitPath:ShitPathInFolder)
	{
		if (ShitPath.Contains(TEXT("Developers"))||
			...
			ShitPath.Contains(TEXT("__ExternalObjects__"))) continue;
		...
		if(!UEditorAssetLibrary::DoesAssetExist(ShitPath)) continue;
	...
	for(const FString& ShitFolder:ShitFolderArr)
	{
		UEditorAssetLibrary::DeleteDirectory(ShitFolder)?
			Counter++:DebugHeader::Print(TEXT("Delete Failed"),FColor::Red);
	}
...
void FEnhancedEditorModV1Module::OnDeleteUnusedAssetsButtonCliked()
{
	...
	const EAppReturnType::Type ConfirmedResult = DebugHeader::ShowMessageDialog(EAppMsgType::YesNo,TEXT("Are you sure to go?"));
	...
	for(const FString& AssetPath:AssetsPathInFolder)
	{
		...
		if (AssetPath.Contains(TEXT("Developers"))||
			...
			AssetPath.Contains(TEXT("__ExternalObjects__"))) continue;
	...
	for(const FString& FolderPath:AssetsPathInFolder)
	{
		...
		if(UEditorAssetLibrary::DoesDirectoryExist(FolderPath) && !UEditorAssetLibrary::DoesDirectoryHaveAssets(FolderPath))
		{
			...
			UEditorAssetLibrary::DeleteDirectory(FolderPath)?
				++Counter : DebugHeader::Print(TEXT("Faild to delete dir: ")+FolderPath,FColor::Red);
		}
